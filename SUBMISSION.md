...
including text that's hard to read, jargon, images missing alt text, and content that's
overwhelming for dyslexic or ADHD readers, and suggests a fix. Every rewrite is human
approved before it ever gets posted.
...
grade level), jargon and unexplained acronyms, images missing meaningful alt text, and
...
with dyslexia or ADHD, for people who speak English as a second language, for anyone
reading quickly on a phone, and for screen reader users when images have no alt text.
Most of the time nobody notices, because the person who wrote the message can read it
fine. The cost lands on whoever's on the other end.
...
terms, and offers a fix that a human has to actively choose to use. That human in the
loop design is deliberate: accessibility tools that automatically edit content erode
trust and can misfire; a tool that suggests and explains builds the habit of writing more
accessibly in the first place.
...
(Flesch Kincaid grade level), jargon and acronyms, missing image alt text, and cognitive
load issues that specifically affect dyslexic and ADHD readers (walls of text, shouting,
...
of an API key. That meant rewriting the tool calling loop against a different function
calling schema format (Gemini's `Schema.from_json_schema` vs. Claude's tool use blocks),
a good reminder that the MCP layer is what actually matters for portability: swapping the
reasoning model out barely touched the MCP server at all.
...
(bullets, text with the shouting removed, a TL;DR) rather than just producing plainer
prose.
...
(not just on request), and swapping the heuristic jargon and cognitive load checks for
something closer to an actual readability and accessibility model.

---

## Devpost checklist status

1. [DONE] Project name, elevator pitch, description, impact statement, drafted above
2. [DONE] Architecture diagram (PNG): `agent/assets/architecture_diagram.png`
3. [DONE] Demo video script (~3 min, narrates MCP host/client/server roles): `DEMO_SCRIPT.md`
4. [DONE] Tags, drafted above
5. [DONE] "Try it out" repo link
6. [HUMAN STEP] Record and upload the ~3 min demo video (follow `DEMO_SCRIPT.md`)
7. [HUMAN STEP] Slack developer sandbox URL
8. [HUMAN STEP] Grant workspace/app access to `slackhack@salesforce.com` and
   `testing@devpost.com`, see checklist below
9. [HUMAN STEP] Image gallery screenshots (up to 15, JPG/PNG/GIF, 3:2 ratio ideal)
10. [HUMAN STEP] Paste everything above into the actual Devpost submission form
...
1. The Agents sidebar showing Cleary with its greeting and three suggested prompts
...
5. A restructured rewrite (bullets and TL;DR) next to the original wall of text input
